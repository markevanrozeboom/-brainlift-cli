...
def load_env():
    ...
    if not function_url or not api_key:
        print("\033[33m⚠️  BrainLift CLI configuration is missing or incomplete.\033[0m")
        print("\033[33mRun 'blm configure' to set up your API endpoint and key.\033[0m")
        
        # Only exit if we're not already running the configure command
        if len(sys.argv) < 2 or sys.argv[1] != 'configure':
            sys.exit(1)
    else:
...
def main():
    ...
    # Load environment variables (skip for configure command)
